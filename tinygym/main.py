...
class Learner:
    ...
    def fit(self, iters=40, steps=16, lr=.01, anneal_lr=True, target_kl=None, **hparams):
        curves = []
        ...
        for i in pbar:
            ...
            metrics = ppo(self.model, opt, obs, values, acts, logprobs, rewards, dones, **hparams)
            #ppo(self.model, opt, obs, values, acts, logprobs, rewards, dones, **hparams)
...
def rollout(env, model, steps, state=None):
    ...
    for i in range(steps):
        with Tensor.test():
            o = Tensor(env.obs, dtype=dtypes.float32)
            ...
            env.step(act.numpy())
    return tuple([Tensor.stack(*xs, dim=1).realize() for xs in [obs, values, acts, logprobs]] +
                 [Tensor(np.stack(xs, axis=1)).cast(dtypes.float32) for xs in [rewards, dones]])
...
def ppo(model, opt, obs, values, acts, logprobs, rewards, dones, bs=8192, gamma=.99, gae_lambda=.95, clip_coef=.1,
    ...
    for o, old_value, act, old_logp, adv, ret in zip(obs, values, acts, logprobs, advs, returns):
        with Tensor.train():
            ...
            opt.step()
        #print(logp.numpy(), entropy.numpy(), value.numpy())
        print(old_value.numpy())
        print(act.numpy())
        print(old_logp.numpy())
        print(adv.numpy(), adv.numpy().mean())
        print(ret.numpy(), ret.numpy().mean())
...
def get_advantages(values, rewards, dones, gamma=.99, gae_lambda=.95):  # see arxiv.org/abs/1506.02438 eq. (16)-(18)
    advs = Tensor.zeros(values.shape).contiguous()
