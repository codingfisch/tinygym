...
class LSTMPolicy:
    def __init__(self, env, n_hidden=64):
        self.encoder = Linear(math.prod(env.obs.shape[1:]), n_hidden)
        self.actor = Linear(n_hidden, env.n_acts)
        self.value_head = Linear(n_hidden, 1)
        self.lstm = None#LSTMCell(n_hidden, n_hidden)
        # self.lstm.bias_hh *= 0
        # self.lstm.bias_ih *= 0
        # qr = QR()
    ...
    def __call__(self, x, state=None, act=None, with_entropy=None):
        ...
        h, c = (h, None)#self.lstm(x, state)
        value = self.value_head(h)[:, 0]
        x = self.actor(h)
        act = sample_multinomial(x.softmax()).squeeze() if act is None else act
...
def sample_multinomial(weights):
    ...
    # rand_vals = torch.randperm(weights.size(0))[:num_samples].float() / weights.size(0)
